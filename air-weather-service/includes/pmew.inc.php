...
function getSun($ICAO) {
         ...
         if (ereg("^([A-Z]{4})$", $ICAO)) {
            ...
            if (file_exists($path_to_icao_db."icao_$firstletter.inc.php")) {
               require ($path_to_icao_db."icao_$firstletter.inc.php");
...
function getFullMoon ($tstamp) {
         ...
         if ((($tstamp < 1860961) || ($tstamp > 2111926199)) && (strtolower($tstamp) != "now")) {
            ...
            require ($path_to_icao_db."moon.inc.php");
            $starter = 0;
...
function readCountrys() {
         global $path_to_icao_db;
         require ($path_to_icao_db."countries.inc.php");
         return $country;
}
...
function getMetarFromBuffer($ICAO) {
         ...
         if (file_exists($path_to_buffer . "metar/" . $ICAO . ".TXT")) {
            ...
            $localhour = date("G", (mktime()+3600*$stationInfo["gmttime"]));
...
function getStationInfo($ICAO) {
         ...
         if (ereg("^([A-Z]{4})$", $ICAO)) {
            ...
            $returner["country"] = $country[$country[$ICAO]];
            ...
            $returner["longitude"] = HMSmaker(abs($longitude[$ICAO])) . $we;
            $returner["latitude"] = HMSmaker(abs($latitude[$ICAO])) . $ns;
...
function getStations($countryindex) {
         ...
         if (ereg("^([A-Z]{2})$", $countryindex)
         && (file_exists($path_to_icao_db."stations_".strtolower($countryindex).".inc.php"))) {
            require($path_to_icao_db."stations_".strtolower($countryindex).".inc.php");
...
function getIssueTime($tafcode,$lang) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^([0-3]{1})([0-9]{1})([0-2]{1})([0-9]{1})([0-5]{1})([0-9]{1})Z$", $tafItemArray[$a])) {
                if (($lang=="EN") || ($lang=="en")) {
                   $aktualhour=1*substr($tafItemArray[$a],2,2);
                   $ampm="AM";
                   if ($aktualhour > 12) {
                      $aktualhour-=12;
                      $ampm="PM";
                   }
                   $returner = "this months " . (1*substr($tafItemArray[$a],0,2)) . numberPrefix(substr($tafItemArray[$a],0,2)) . ", ";
...
function getValidTime($tafcode,$lang) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^([0-3]{1})([0-9]{1})([0-2]{1})([0-9]{1})([0-2]{1})([0-9]{1})$", $tafItemArray[$a])) {
...
function getShortValidTime($tafcode,$lang) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^([0-2]{1})([0-9]{1})([0-2]{1})([0-9]{1})$", $tafItemArray[$a])) {
...
function getFmTime($tafcode, $lang) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^FM([0-9]{4})$", $tafItemArray[$a])) {
...
function getWind($tafcode) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^(.+KT)|(.+MPS)|(.+KMH)$", $tafItemArray[$a])) {
                ...
                if (ereg("^.+KT$", $tafItemArray[$a])) {
                ...
                } else if (ereg("^.+MPS$", $tafItemArray[$a])) {
                   if (strpos($tafItemArray[$a],"G")) {
                      $explodedItemArray=explode("G",$tafItemArray[$a]);
                   ...
                   if (strpos($tafItemArray[$a],"G")) {
                      ...
                      $returner["wkt"] = round(substr($tafItemArray[$a],3,2)/1.8);
                      $returner["gkt"] = round(substr($tafItemArray[$a],3,2)/1.8);
                   }
                }
                $returner["wkm"] = round($returner["wkt"]*1.8);
                $returner["wms"] = round(0.5*$returner["wkt"]);
                $returner["wbf"] = windBeaufort($returner["wkt"]);
                $returner["gkm"] = round($returner["gkt"]*1.8);
                $returner["gms"] = round(0.5*$returner["gkt"]);
...
function getMetarTemp($tafcode) {
         ...
         for ($a=0;$a<count($tafItemArray);$a++) {
             ...
             $tafItemArray[$a]=str_replace("M","-",$tafItemArray[$a]);
             if (ereg("-*([0-9]{2})/-*([0-9]{2})$", $tafItemArray[$a])) {
...
function getTafTemp($tafcode,$lang) {
         ...
         for ($a=0;$a<count($tafItemArray);$a++) {
             $tafItemArray[$a]=str_replace("m","-",$tafItemArray[$a]);
             $tafItemArray[$a]=str_replace("M","-",$tafItemArray[$a]);
             if (ereg("T(-)*([0-9]{2})/([0-9]{2})Z$", $tafItemArray[$a])) {
...
function getPressure($tafcode) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^Q([0-9]{4})$", $tafItemArray[$a])) {
                $returner = 1*substr($tafItemArray[$a],1,4);
                break;
             } else if (ereg("^A([0-9]{4})", $tafItemArray[$a])) {
                $returner = round(0.3386389*substr($tafItemArray[$a],1,4));
                break;
             } else if (ereg("^QNH([0-9]{4})(INS)?$", $tafItemArray[$a])) {
...
function getVisibility($tafcode) {
         ...
         for ($a=$type_Array["init"];$a<count($tafItemArray);$a++) {
             if (ereg("^((0([0-9]{2})0)|((1|2)([0-9]{1})(0|5)0)|(([3-9]{1})(0|5)00)|(9999))$", $tafItemArray[$a])) {
                $returner=1*$tafItemArray[$a];
             } else if (ereg("^((P|M)?)([0-9]+)((/([0-9]{1}))?)SM$", $tafItemArray[$a])) {
                if (ereg("^P", $tafItemArray[$a])) {
                   $returner = "more than ";
                   $tafItemArray[$a] = substr($tafItemArray[$a],1);
                } else if (ereg("^M", $tafItemArray[$a])) {
                   $returner = "less than ";
                   $tafItemArray[$a] = substr($tafItemArray[$a],1);
                }
                ...
                break;
             } else if (ereg("^([0-9]+)KM$", $tafItemArray[$a])) {
...
function getMetarRVR($tafcode,$number) {
         ...
         $aktualRailway = 0;
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^R([0-9]{2})(L|C|R)?/(M|P)?([0-9]{4})((V|VP|VM)([0-9]{4}))?(U|D|N)?$", $tafItemArray[$a])) {
...
function getWeatherGrafic($tafcode,$icao) {
         ...
         if (ereg("(mist)|(fog)|(smoke)|(haze)|(dust)|(sand)|(ash)", $weatherCond)) $pic = "dust";
         if (ereg("(drizzle)", $weatherCond)) $pic = "drizzle";
         if (ereg("(rain)", $weatherCond)) $pic = "rain";
         if (ereg("(light rain)", $weatherCond)) $pic = "lightrain";
         if (ereg("(snow)|(ice)|(hail)", $weatherCond)) $pic = "snow";
         if (ereg("(thunderstorm)", $weatherCond)) $pic = "thunderstorm";
         if (ereg("(towering)", $skyCond)) $pic = "thunderstorm";
...
function getSky($tafcode,$lang) {
         ...
         if (($lang == "DE") || ($lang == "de")) {
            $returner = str_replace("no significant clouds below 5,000 ft","keine wesentliche Bew&ouml;lkung unter 1.500 Meter",$returner);
            $returner = str_replace("sky clear","wolkenlos",$returner);
            $returner = str_replace("few clouds","leichte Bew&ouml;lkung",$returner);
            $returner = str_replace("partly cloudy","teilweise bew&ouml;lkt",$returner);
            $returner = str_replace("mostly cloudy","&uuml;berwiegend bew&ouml;lkt",$returner);
...
function getClouds($tafcode,$number,$lang) {
             ...
             if (($first3=="NSC") || ($first3=="CLR") || ($first3=="SKC") || ($first3=="FEW") || ($first3=="SCT") || ($first3=="BKN") || ($first3=="OVC")) {
                if ($number==$aktualCloudBase++) {
                   ...
                   if (($first3 == "SKC") || ($first3 == "CLR")) {
                      ...
                      if (($lang == "DE") || ($lang == "de")) {
                         $returner["sky"] .= number_format($returner["meters"], 0, ",", ".") . " Meter H&ouml;he";
                      } else {
                         $returner["sky"] .= number_format($returner["feet"], 0, ".", ",") . " feet";
                      }
                      if (strpos($tafItemArray[$a],"CB")) {
                         $returner["cumulus"] = "yes";
                         $returner["sky"] .= " with cumulonimbus clouds";
                      } else if (strpos($tafItemArray[$a],"TCU")) {
                         $returner["cumulus"] = "towering";
                         $returner["sky"] .= " with towering cumulus clouds";
                      } else {
                         $returner["cumulus"] = "no";
                      }
                   }
         ...
         if (($lang == "DE") || ($lang == "de")) {
            $returner = str_replace("sky clear","wolkenlos",$returner);
            $returner = str_replace("no significant clouds below 5,000 ft","keine wesentliche Bew&ouml;lkung unter 1.500 Meter",$returner);
            $returner = str_replace("few clouds at","leichte Bew&ouml;lkung in",$returner);
            $returner = str_replace("partly cloudy at","teiweise bew&ouml;lkt in",$returner);
            $returner = str_replace("mostly cloudy at","&uuml;berwiegend bew&ouml;lkt in",$returner);
            $returner = str_replace("sky overcast at","bedeckter Himmel in",$returner);
            $returner = str_replace("not reported","k/A",$returner);
            $returner = str_replace("with cumulonimbus clouds","mit Cumulonimbus Wolken",$returner);
            $returner = str_replace("with towering cumulus clouds","mit Gewitterwolken",$returner);
         }
         if (isset($returner)) {
            return $returner;
         } else {
...
function getConditions($tafcode,$lang) {
         $tafItemArray=clearTAF($tafcode);
         for ($a=0;$a<count($tafItemArray);$a++) {
             if (ereg("^((\+)|(-))?((NSW)|(DZ)|(RA)|(SN)|(SQ)|(IC)|(PE)|(BR)|(FG)|(FU)|(HZ)|(VA)|(DU)|(SA)|(DS)|(FC)|(MI)|(BC)|(DR)|(BL)|(TS)|(FZ)|(VC)|(SS)|(SG)|(SH)|(GR)|(GS))*$", $tafItemArray[$a]) && ($getConditions == "yes")) {
                if (ereg("^\+", $tafItemArray[$a])) {
                   $tafItemArray[$a] = substr($tafItemArray[$a],1);
                   $returner = "heavy";
                } else if (ereg("^-", $tafItemArray[$a])) {
                   $tafItemArray[$a] = substr($tafItemArray[$a],1);
                   $returner = "light";
                }
             ...
             } else if (ereg("^PROB([0-9]{2})$", $tafItemArray[$a]) || ereg("^BECMG$", $tafItemArray[$a]) || ereg("^TEMPO$", $tafItemArray[$a])) {
                $getConditions = "yes";
         ...
         if (($lang=="de")||($lang=="DE")) {
            $returner = str_replace("heavy", "verst&auml;rkt", $returner);
            $returner = str_replace("light", "vereinzelt", $returner);
            $returner = str_replace("rain showers", "Regenschauer", $returner);
            $returner = str_replace("snow showers", "Schneeschauer", $returner);
            $returner = str_replace("hail showers", "Hagelschauer", $returner);
            $returner = str_replace("showers of small hail", "feine Hagelschauer", $returner);
            $returner = str_replace("waft of mist", "Nebelschwaden", $returner);
            $returner = str_replace("waft of smoke", "Rauchschwaden", $returner);
            $returner = str_replace("thunderstorm with rain", "Gewitter und Regen", $returner);
            $returner = str_replace("thunderstorm with hail", "Gewitter mit Hagelschlag", $returner);
            $returner = str_replace("thunderstorm with small hail", "Gewitter mit feinem Hagel", $returner);
            $returner = str_replace("with", "mit", $returner);
            $returner = str_replace("drizzle", "Niesel", $returner);
            $returner = str_replace("rain", "Regen", $returner);
            $returner = str_replace("snow", "Schnee", $returner);
            $returner = str_replace("squalls", "B&ouml;en", $returner);
            $returner = str_replace("diamond dust", "Eiskristalle", $returner);
            $returner = str_replace("ice pellets", "Eisk&ouml;rner", $returner);
            $returner = str_replace("mist", "feuchter Dunst", $returner);
            $returner = str_replace("fog", "Nebel", $returner);
            $returner = str_replace("smoke", "Rauch", $returner);
            $returner = str_replace("haze", "trockener Dunst", $returner);
            $returner = str_replace("volcanic ash", "Ascheregen/Vulkanasche", $returner);
            $returner = str_replace("widespread dust", "(weit-)verbreiteter Nebel", $returner);
            $returner = str_replace("sand", "Sand", $returner);
            $returner = str_replace("duststorm", "Sandsturm", $returner);
            $returner = str_replace("funnel cloud", "Wolkentrichter", $returner);
            $returner = str_replace("shallow fog", "Bodennebel", $returner);
            $returner = str_replace("shallow", "niedrige(r)", $returner);
            $returner = str_replace("patches", "Schwaden", $returner);
            $returner = str_replace("drifting", "treibende(r)", $returner);
            $returner = str_replace("blowing", "blasende(r)", $returner);
            $returner = str_replace("thunderstorm", "Gewitter", $returner);
            $returner = str_replace("supercooled (freezing)", "gefrierende(r)", $returner);
            $returner = str_replace("outside the airport", "ausserhalb der Airbase", $returner);
            $returner = str_replace("sandstorm", "Sandsturm", $returner);
            $returner = str_replace("snow grains", "Schnee-/Eisk&ouml;rner", $returner);
            $returner = str_replace("showers", "Schauer", $returner);
            $returner = str_replace("hail", "Hagel", $returner);
            $returner = str_replace("small hail", "feiner Hagel", $returner);
         }
         if (isset($returner)) {
            return $returner;
         } else {
...
function displayEasyWeatherJSON($METAR,$lang) {
         
         $metarParts = getMetarParts($METAR);
         for ($a=0;$a<count($metarParts);$a++) {
             $weatherType = getMetarType($metarParts[$a]);
             $data["metar"] = $METAR;
             $data["weather_type"]["value"] = $weatherType["type"];
             switch ($weatherType["type"]) {
                    case "main":
                         if ($station=getStationInfo(getICAO($metarParts[$a]))) {
                           $data["weather_type"]["station"] =  $station["station"];
                         }
                         $returner .= "<b>Actual weather ";
                         if ($issuetime = getIssueTime ($metarParts[$a], $lang)) {
                            $data["weather_type"]["issuetime"] =  $issuetime;
                            $data["weather_type"]["desc"] = 'Actual Weather from '.$issuetime;
                         }
                    break;
                    case "becmg":
                         $data["weather_type"]["desc"] = '2 hours trend';
                    break;
                    case "tempo":
                         $data["weather_type"]["desc"] = '2 hours trend: temporarily';
                    break;
                    case "nosig":
                         $data["weather_type"]["desc"] = '2 hours trend: no changes';
                    break;
             }
             if ($wind = getWind($metarParts[$a])) {
                   $data["wind"]["direction"] = $wind["dir"];
                   $data["wind"]["degrees"] = $wind["deg"];
                   $data["wind"]["force"] = $wind["wbf"];
                   $data["wind"]["speed"] = $wind["wkt"];
                   $data["wind"]["gust"] = $wind["gbf"];
                   $data["wind"]["beaufort"] = $wind["gkt"];

                if ($wind["dir"]=="variable") {
                   $data["wind"]["desc"] .= "Wind direction changing";
                } else {
                   $data["wind"]["desc"] .= "Winds from ".$wind["dir"]." (".$wind["deg"]."°) ";
                }
                
                $data["wind"]["desc"] .= "force ".$wind["wbf"]." beaufort (".$wind["wkt"]." knots)";
                if ($wind["gkt"] > $wind["wkt"]) {
                   $data["wind"]["desc"] .= " with gusts up to ".$wind["gbf"]." beaufort (".$wind["gkt"]." knots)";
                }
             }
             if ($temp = getMetarTemp($metarParts[$a])) {
                $data["temperature"]["value"] = $temp;
                $data["temperature"]["desc"] = "Temperature: $temp deg C";
             }
             if ($dew = getMetarDewpoint($metarParts[$a])) {
                $data["dewpoint"]["value"] = $dew;
                $data["dewpoint"]["desc"] = "Dewpoint: $dew deg C";
             }
             if ($hum = getMetarHumidity($metarParts[$a])) {
                $data["humidity"]["value"] = $hum;
                $data["humidity"]["desc"] = "Humidity: $hum %";  
             }
             if ($press = getPressure($metarParts[$a])) {
                $data["pressure"]["value"] = $press;
                $data["pressure"]["desc"] = "Air pressure: $press hPa";
             }
             if ($vis = getVisibility($metarParts[$a])) {
                $data["visibility"]["value"] = $vis;
                if ($vis == 9999) {
                   $data["visibility"]["desc"] = "more than 10 km";
                } else if ($vis == 8888) {
                   $data["visibility"]["desc"] = "more than 10 km, no clouds below 5000 ft, no precipitation";
                } else if ($vis < 10000) {
                   $data["visibility"]["desc"] = "$vis meters";
                } else if ($vis >= 10000) {
                   $data["visibility"]["desc"] = round($vis/1000) . " km";
                }
             }
             if ($sky = getSky($metarParts[$a],$lang)) {
                $returner .= "$sky,\n";
             ...
             if ($cond = getConditions($metarParts[$a],$lang)) {
                $data["cond"]["value"] = $cond;
                if ($cond == "nsw") {
                   if ($weatherType["type"]=="main") {
                      $data["cond"]["desc"] = "no significant weather\n";
                      $oldcond = "no significant weather\n";     // if this appears the meteorologist
                   } else {                                      // must be stupid: NSW will breakup ROFL
                      $data["cond"]["desc"] = "$oldcond will breakup\n";
                   }
                } else {
                   $data["cond"]["desc"] = "$cond";
                   $oldcond = $cond;
...
function getTafParts($tafcode) {
         ...
         for ($a=0;$a<count($tafItemArray);$a++) {
             ...
             || ereg("^PROB([0-9]{2})$", $tafItemArray[$a]) || ereg("^FM([0-9]{4})$",$tafItemArray[$a]))
             && ($a > ($lastTafindex+2))) {
